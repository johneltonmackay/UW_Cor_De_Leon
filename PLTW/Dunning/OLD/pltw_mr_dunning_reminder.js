   ...
   function (record, runtime, email, file, render, nsutil, search) {
      ...
      function getInputData() {
         ...
         try {
            ...
            var stMainFolder = runtime.getCurrentScript().getParameter('custscript_reminder_pdffolder');
            createFolder(stMainFolder, stToday);

            var ssInvoicesFirst_id = runtime.getCurrentScript().getParameter('custscript_reminderinvoices_ss');
      ...
      function reduce(context) {
         ...
         try {
            ...
            var arrInvoicesId = [];
            var arrReceivers = [];
            var stSender = runtime.getCurrentScript().getParameter('custscript_reminder_email_sender');
            var stEmailTemplate = runtime.getCurrentScript().getParameter('custscript_reminder_email_template');
            var stInvoiceTemplatePDFId = runtime.getCurrentScript().getParameter('custscript_reminder_inv_pdf_id');
            ...
            if (bPrintInvoice) {
               ...
               var stReminderLetterURL = runtime.getCurrentScript().getParameter('custscript_reminder_letter');
            ...
            if (bEmailInvoice && !isEmpty(stSender) && !isEmpty(arrReceivers) && !isEmpty(stEmailSubject) && !isEmpty(stEmailContent)) {
               email.send({
                  author: stSender,
      ...
      function createFolder(stMainFolder, stToday) {
         ...
         try {
            ...
            var stReminderFolders = runtime.getCurrentScript().getParameter('custscript_reminder_folders');
